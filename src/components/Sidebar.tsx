...
export function Sidebar({ isOpen, onToggle, onSelectChat }: { isOpen: boolean, onToggle: () => void, onSelectChat?: (id: string) => void }) {
  ...
  const handleChatSelect = (id: string) => {
    ...
    if (window.innerWidth < 768) {
      onToggle(); // Close sidebar on mobile after selecting a chat
    }
  };
  ...
  return (
    <>
      ...
      {/* Desktop Standalone Open Button (when sidebar is closed) */}
      {!isOpen && (
      ...
      {/* Sidebar Content */}
      <div 
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-sidebar border-r border-sidebar-border h-screen flex flex-col transition-transform duration-300 ease-in-out md:sticky md:top-0 ${
